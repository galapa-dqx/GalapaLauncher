...
public sealed class InjectorBootstrapTests
{
    ...
    public void X86ApcThunkCarriesExpectedPointersAndCallingConventionCleanup()
    {
        ...
        Assert.Equal(new byte[] { 0x5D, 0xC2, 0x04, 0x00 }, thunk[46..50]);

        Assert.Equal(46, ShortBranchTarget(thunk, 18));
        Assert.Equal(46, ShortBranchTarget(thunk, 35));
        Assert.Equal(
            new byte[] { 0x85, 0xC0, 0x74 },
            thunk[15..18]);
        Assert.Equal(
            new byte[] { 0x85, 0xC0, 0x74 },
            thunk[32..35]);
