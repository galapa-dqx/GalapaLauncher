...
/// <summary>
/// A <see cref="BinaryReader"/> that folds every byte it reads into a running
/// <see cref="ZiPatchCrc32"/>. Used to verify the trailing CRC of each chunk when a
/// patch is opened with checksum validation enabled.
/// </summary>
internal sealed class ChecksumBinaryReader(Stream input) : BinaryReader(input)
{
    private readonly ZiPatchCrc32 _crc32 = new();

    public void InitCrc32() => _crc32.Init();

    public uint GetCrc32() => _crc32.Checksum;
    ...
    public override double ReadDouble() => BitConverter.ToDouble(ReadBytes(sizeof(double)), 0);
}
