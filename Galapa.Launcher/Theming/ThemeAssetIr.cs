...
public sealed class ThemeSvgIrParser
{
    ...
    private static IReadOnlyList<SvgShapeIr> ParseShapes(XElement container)
    {
        ...
        var fill = ThemePaintIr.Parse(container.Attribute("fill")?.Value ?? "black");
        var stroke = ThemePaintIr.Parse(container.Attribute("stroke")?.Value);
    ...
    private static void ParseElement(XElement element, ThemeMatrix parentTransform, ThemePaintIr inheritedFill,
        ThemePaintIr inheritedStroke, double inheritedStrokeWidth, ICollection<SvgShapeIr> output)
    {
        var transform = ParseTransform(element.Attribute("transform")?.Value) * parentTransform;
        var fill = element.Attribute("fill") is { } fillAttribute
            ? ThemePaintIr.Parse(fillAttribute.Value)
            : inheritedFill;
        var stroke = element.Attribute("stroke") is { } strokeAttribute
            ? ThemePaintIr.Parse(strokeAttribute.Value)
            : inheritedStroke;
        ...
        SvgShapeIr? shape = element.Name.LocalName switch
        {
            "path" when element.Attribute("d")?.Value is { Length: > 0 } data =>
                new SvgShapeIr(SvgGeometryKind.Path, data, default, 0, 0, 0, 0, fill, stroke, strokeWidth, transform),
            "circle" when Number(element.Attribute("r")?.Value, 0) is > 0 and var radius =>
                new SvgShapeIr(SvgGeometryKind.Circle, null,
                    new ThemeRect(Number(element.Attribute("cx")?.Value, 0) - radius,
                        Number(element.Attribute("cy")?.Value, 0) - radius, radius * 2, radius * 2),
                    radius, radius, 0, 0, fill, stroke, strokeWidth, transform),
            "line" => new SvgShapeIr(SvgGeometryKind.Line, null,
                new ThemeRect(Number(element.Attribute("x1")?.Value, 0), Number(element.Attribute("y1")?.Value, 0), 0, 0),
                0, 0, Number(element.Attribute("x2")?.Value, 0), Number(element.Attribute("y2")?.Value, 0),
                fill, stroke, strokeWidth, transform),
            "rect" => new SvgShapeIr(SvgGeometryKind.Rectangle, null,
                ...
                0, 0, fill, stroke, strokeWidth, transform),
            _ => null
        };
